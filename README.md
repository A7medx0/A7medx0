<div align="center">

# 👋 Hi, I'm A7hmedx0

### 💻 Java Developer | Front-End Developer | Computer Engineering Student

<p>
I enjoy building applications, learning new technologies, and turning ideas into practical projects.
</p>

<p>
Currently focused on <b>Java Development</b>, <b>Front-End Development</b>, and <b>Cybersecurity</b>.
</p>

<br>

<img src="https://github.com/SP-XD/SP-XD/blob/main/images/hellocoders_rounded.gif?raw=true" width="60%"/>

<br><br>

![Profile Views](https://komarev.com/ghpvc/?username=A7hmedx0\&style=flat\&color=orange\&label=PROFILE+VIEWS)

</div>

---

## 🚀 About Me

* 🎓 Computer Engineering Student
* ☕ Java Developer
* 🌐 Front-End Developer
* 🔐 Interested in Cybersecurity & Ethical Hacking
* 🤖 Interested in Artificial Intelligence
* 💡 Passionate about Software Development
* 🧠 Always learning and improving my programming skills
* 🛠️ I enjoy building projects and solving technical problems

---

## 💻 Languages & Technologies

### Programming Languages

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat\&logo=openjdk\&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat\&logo=javascript\&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat\&logo=python\&logoColor=white)

### Front-End

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat\&logo=html5\&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat\&logo=css3\&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat\&logo=javascript\&logoColor=black)

### Database

![PL/SQL](https://img.shields.io/badge/PL%2FSQL-F80000?style=flat\&logo=oracle\&logoColor=white)
![Oracle Database](https://img.shields.io/badge/Oracle%20Database-F80000?style=flat\&logo=oracle\&logoColor=white)

### Tools

![Git](https://img.shields.io/badge/Git-E44C30?style=flat\&logo=git\&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat\&logo=github\&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat\&logo=visual-studio-code\&logoColor=white)
![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ%20IDEA-000000?style=flat\&logo=intellij-idea\&logoColor=white)

---

## ☕ Java Development

My main programming language is **Java**.

I'm interested in:

* Object-Oriented Programming (OOP)
* Data Structures & Algorithms
* JavaFX
* Exception Handling
* File Handling
* Database Connectivity
* Software Development

```java
public class A7hmedx0 {

    public static void main(String[] args) {

        String developer = "A7hmedx0";
        String mainLanguage = "Java";
        String focus = "Software Development";

        System.out.println("Keep learning. Keep building. 🚀");
    }
}
```

---

## 🌐 Front-End Development

I enjoy creating clean and interactive web interfaces using:

* HTML5
* CSS3
* JavaScript
* Responsive Web Design

---

## 🔐 Cybersecurity

I'm also interested in **Cybersecurity and Ethical Hacking**.

Currently exploring:

* Networking Fundamentals
* Linux
* Web Security
* OWASP
* OSINT
* Nmap
* Vulnerability Assessment
* Security Fundamentals

---

## 📌 Featured Projects

### ✈️ Gaza International Airport Management System (GIAMS)

A Java-based system designed to manage airport operations and information.

**Technologies:**

`Java` `OOP` `JavaFX` 

---

### 🗃️ Database Projects

Projects involving:

* Oracle Database
* PL/SQL
* SQL
* Database Design
* Database Auditing
* Triggers

---

## 📚 Currently Learning

```text
Java
 ├── OOP
 ├── Data Structures
 ├── JavaFX
 └── Advanced Java

Front-End
 ├── HTML
 ├── CSS
 └── JavaScript

Cybersecurity
 ├── Linux
 ├── Networking
 ├── Web Security
 └── Ethical Hacking

Database
 └── PL/SQL
```

---

## 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=A7hmedx0&theme=tokyonight&hide_border=true"/>

</div>

</div>

---

## 🐍 Contribution Graph

<div align="center">

![GitHub Snake](https://raw.githubusercontent.com/Platane/snk/output/github-contribution-grid-snake.svg)

</div>

---

## 🤝 Let's Connect

<div align="center">

If you're interested in programming, cybersecurity, or technology, feel free to connect with me.

### **Keep Coding. Keep Learning. Keep Building. 🚀**

</div>

